(define-module (sph upcoming))

(use-modules (ice-9 regex) (rnrs eval)
  (rnrs io simple) (sph)
  (rnrs io ports) (sph alist)
  (sph hashtable) (sph io)
  (sph list) (sph other)
  (sph record) (sph server)
  (sph stream) (sph time)
  (sph process) (sph time string)
  (sph time utc) (srfi srfi-2)
  (srfi srfi-41) (srfi srfi-1) ((sph other) #:select (procedure->cached-procedure)))

(export config-cache-config config-cache-get
  config-cache-mtime config-cache-update
  event-data event-end
  event-id event-start
  u-row u-row-data
  u-row-diff-end u-row-diff-start
  u-row-duration u-row-end
  u-row-id u-row-ppf
  u-row-start upcoming
  upcoming-config-cached upcoming-config-cached-static
  upcoming-config-get upcoming-config-path
  upcoming-config-variables upcoming-doc-config-example
  upcoming-doc-config-syntax upcoming-events upcoming-table)
...
(define read-ymd-ks
  (let (regexp (make-regexp (string-append "^(" date-pattern ")( " ks-pattern ")?$")))
    (l (a) "string -> false/integer:utc-time"
      (and-let* ((m (regexp-exec regexp a))) "epoch time, relative time, has epoch time?"
        (list (ns->s (utc-from-ymd (match:substring m 1)))
          (let (m-2 (match:substring m 2)) (if m-2 (ks->s (string->number (string-drop m-2 1))) 0))
          #t)))))
...
(define upcoming-datetime-readers (list read-ymd-ks read-hms read-ymd-hms))
...
(define (day-first-login-time)
  (let ((path (string-append (getenv "HOME") "/.config/sph/login-times")) (time (utc-current)))
    (or
      ...
      (let (boot-time (inexact->exact (truncate (s->ns (- (ns->s time) (os-seconds-since-boot))))))
        (string-append (utc->ymd boot-time) " " (utc-elapsed-day-string boot-time))))))

"symbol -> any/procedure"

(define upcoming-config-variables
  (alist-q uptime os-seconds-since-boot
    uptime-start os-seconds-at-boot login-time day-first-login-time))
...
(define (config-eval line-data variables config-env) "evaluate quasiquoted config data"
  (let*
    ( (variable-names (map first variables))
      ...
      (template (eval (list (q lambda) variable-names (list (q quasiquote) line-data)) config-env)))
...
(define* (upcoming-config-cached #:optional config-path variables config-env)
  ...
  (let
    ( (variables (or variables upcoming-config-variables)) (env (or config-env upcoming-config-env))
